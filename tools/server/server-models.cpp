...
void server_models_routes::init_routes() {
    ...
    this->get_moe_hot_cache = [this](const server_http_req & req) {
        std::string name = req.get_param("model");
        auto error_res = std::make_unique<server_http_res>();
        ...
        if (selected_count == 0) {
            res_err(error_res, format_error_response(
                "no running model found for /moe-hot-cache",
                ERROR_TYPE_INVALID_REQUEST));
            return error_res;
        }
        ...
        if (selected_count > 1) {
            res_err(error_res, format_error_response(
                "multiple running models found for /moe-hot-cache; pass ?model=<name>",
                ERROR_TYPE_INVALID_REQUEST));
            return error_res;
        }
    ...
    this->post_moe_hot_cache = [this](const server_http_req & req) {
        std::string name = req.get_param("model");
        auto error_res = std::make_unique<server_http_res>();
