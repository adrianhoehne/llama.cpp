...
|---|---|---|---|---|
| `LLAMA_ARG_CPU_MOE` | `--cpu-moe` / `cpu-moe = true` | Boolean | off | Praktische Voraussetzung fuer den Hot-Cache-Pfad. Haelt die normalen MoE-Expertentensoren auf CPU/RAM, damit `--moe-hot-cache` nur die ausgewaehlten Hot-Experts zusaetzlich in VRAM kopiert und der Cold-Pfad kontrolliert auf CPU bleibt. |
...
| `LLAMA_ARG_MOE_HOT_CACHE_LAYER_CURVE`; intern auch `LLAMA_MOE_HOT_CACHE_LAYER_CURVE` | `--moe-hot-cache-layer-curve N` / `--moe-hot-cache-qwen-layer-curve N` / `moe-hot-cache-layer-curve = N` | Float `0.0` bis `1.0` | `0.5` | Gewichtet Layer mit hoeherem Zeitdruck bei initialer Auswahl und dynamischem Update staerker. Primaer fuer Qwen35Moe; Gemma4 hat zusaetzlich `LLAMA_MOE_HOT_CACHE_GEMMA4_LAYER_CURVE`. Alte spezifische Env-Aliase wie `LLAMA_MOE_HOT_CACHE_QWEN_LAYER_CURVE` werden noch als Fallback gelesen. |
| `LLAMA_ARG_MOE_HOT_CACHE_WEIGHTING`; intern auch `LLAMA_MOE_HOT_CACHE_WEIGHTING` | `--moe-hot-cache-weighting MODE` / `moe-hot-cache-weighting = MODE` | `flat`, `pressure`, `smooth`, `smooth-pressure`, `capped`, `capped-pressure`, `soft-pressure`, `time`, `moe-time`, `decode-time`, `balanced`, `rank`, `layer-rank` | `flat` | Waehlt den Ranking-Modus fuer initiale Hot-Cache-Auswahl und dynamisches Update. `flat` verteilt das Budget moeglichst gleichmaessig ueber die beobachteten Layer, indem Experten zuerst je Layer nach Hits sortiert und dann gleiche Raenge ueber die Layer interleaved werden. `pressure` stellt den vorherigen druckgewichteten Default wieder her. |
| `LLAMA_ARG_MOE_HOT_CACHE_PP_REDUCE_MERGE`; intern auch `LLAMA_MOE_HOT_CACHE_PP_REDUCE_MERGE` | `--moe-hot-cache-pp-reduce-merge MODE` / `moe-hot-cache-pp-reduce-merge = MODE` | `off`, `on`, `auto` | `off` | Experimenteller Prompt-Processing-Hebel. Reduziert Hot- und Cold-Branch jeweils auf `[n_embd, n_tokens]`, bevor beide Branches gemerged werden. Dadurch wird der grosse Slot-Merge fuer batched PP vermieden. `auto` aktiviert erst ab groesseren PP-Batches. Decode bleibt unveraendert. |
...
| `LLAMA_MOE_HOT_CACHE_BRANCH_REDUCE_MERGE` | Kein Argument | Boolean | on | Aktiviert den Branch-Reduce-Merge-Pfad als Gemma4-spezifischen Vergleichs-/Fallback-Hebel. Im normalen Decode ist inzwischen der direkte Decode-Merge der primaere Gemma4-Pfad; Branch-Reduce-Merge wird relevant, wenn der direkte Decode-Merge deaktiviert ist. Qwen35Moe setzt diesen Profil-Schalter explizit aus. |
...
| `LLAMA_MOE_HOT_CACHE_DECODE_DIRECT_MERGE` | Kein Argument | Boolean | on, aber architekturabhaengig | Decode-Optimierung fuer Qwen35Moe und Gemma4: merged Single-Token-Decode direkt in das finale Ergebnis, statt erst groessere Slot-Zwischenformen zu erzeugen. |
...
| `LLAMA_MOE_HOT_CACHE_COLD_PREFIX_SUM` | Kein Argument | Boolean | on, aber architekturabhaengig | Cold-Decode-Optimierung: behandelt valide Cold-Slots als kompakten Prefix und reduziert nur diesen Bereich. Fuer Qwen35Moe und Gemma4 aktivierbar; fuer andere Architekturen nur nach expliziter Profil-Freigabe. |
