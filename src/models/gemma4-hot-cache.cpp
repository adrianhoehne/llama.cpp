...
static constexpr double GEMMA4_LAYER_CURVE_DEFAULT = 0.50;
static constexpr double GEMMA4_HOT_STICKY_BONUS = 0.05;

static double gemma4_layer_curve() {
    static const double value = []() {
        const char * env = std::getenv("LLAMA_MOE_HOT_CACHE_GEMMA4_LAYER_CURVE");
        if (env == nullptr || env[0] == '\0') {
            return GEMMA4_LAYER_CURVE_DEFAULT;
        }
...
static double layer_weight(const llama_moe_hot_cache_layer_observation & layer, double avg_pressure) {
    const double curve = gemma4_layer_curve();
    if (curve <= 0.0) {
        return 1.0;
    }
    ...
    for (const auto & layer : observations) {
        ...
        for (const auto & expert : layer.experts) {
            ...
            if (layer.has_branch_counts && expert.hot > 0) {
                score += (long double) expert.hot * (long double) GEMMA4_HOT_STICKY_BONUS;
            }
