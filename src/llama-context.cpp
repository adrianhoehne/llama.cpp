    ...
    // reserve pp (prompt processing) graph first so that buffers are only allocated once
    {
        ...
        if (!gf) {
            if (cparams.pipeline_parallel) {
                ...
                gf = graph_reserve(n_tokens, n_seqs, n_outputs_pp, mctx.get(), false, nullptr,
                        LLM_GRAPH_PHASE_PROMPT_PROCESSING);
            }
            if (!gf) {
...
bool llama_context::memory_update(bool optimize) {
    ...
    // if the memory module did any computation, we have to reserve a new worst-case graph
    {
        ...
        auto * gf = graph_reserve(n_tokens, n_seqs, n_outputs_max, mctx.get(), false, nullptr,
                LLM_GRAPH_PHASE_PROMPT_PROCESSING);
        if (!gf) {
...
uint32_t llama_context::graph_max_nodes(uint32_t n_tokens) const {
    if (model.arch == LLM_ARCH_QWEN3NEXT || model.arch == LLM_ARCH_MELLUM || model.arch == LLM_ARCH_KIMI_LINEAR || model.arch == LLM_ARCH_QWEN35 || model.arch == LLM_ARCH_QWEN35MOE) {
