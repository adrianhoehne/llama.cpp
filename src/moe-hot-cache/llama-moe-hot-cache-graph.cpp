...
static ggml_custom3_op_t llama_moe_hot_cache_select_worklist_op(llama_moe_hot_cache_worklist_order order) {
    switch (order) {
        case llama_moe_hot_cache_worklist_order::expert_major:
            return llama_qwen35moe_hot_cache_build_worklist_op<llama_moe_hot_cache_worklist_order::expert_major>;
        case llama_moe_hot_cache_worklist_order::token_aligned:
            return llama_qwen35moe_hot_cache_build_worklist_op<llama_moe_hot_cache_worklist_order::token_aligned>;
        case llama_moe_hot_cache_worklist_order::token_major:
            return llama_qwen35moe_hot_cache_build_worklist_op<llama_moe_hot_cache_worklist_order::token_major>;
    }
    GGML_ABORT("invalid MoE hot-cache worklist order");
}

static ggml_custom2_op_t llama_moe_hot_cache_select_worklist_from_logits_op(llama_moe_hot_cache_worklist_order order) {
    switch (order) {
        case llama_moe_hot_cache_worklist_order::expert_major:
            return llama_qwen35moe_hot_cache_build_worklist_from_logits_op<llama_moe_hot_cache_worklist_order::expert_major>;
        case llama_moe_hot_cache_worklist_order::token_aligned:
            return llama_qwen35moe_hot_cache_build_worklist_from_logits_op<llama_moe_hot_cache_worklist_order::token_aligned>;
        case llama_moe_hot_cache_worklist_order::token_major:
            return llama_qwen35moe_hot_cache_build_worklist_from_logits_op<llama_moe_hot_cache_worklist_order::token_major>;
    }
    GGML_ABORT("invalid MoE hot-cache worklist order");
}
...
static ggml_custom3_op_t llama_moe_hot_cache_select_worklist_from_biased_logits_op(llama_moe_hot_cache_worklist_order order) {
    switch (order) {
        case llama_moe_hot_cache_worklist_order::expert_major:
            return llama_moe_hot_cache_build_worklist_from_biased_logits_op<llama_moe_hot_cache_worklist_order::expert_major>;
        case llama_moe_hot_cache_worklist_order::token_aligned:
            return llama_moe_hot_cache_build_worklist_from_biased_logits_op<llama_moe_hot_cache_worklist_order::token_aligned>;
        case llama_moe_hot_cache_worklist_order::token_major:
            return llama_moe_hot_cache_build_worklist_from_biased_logits_op<llama_moe_hot_cache_worklist_order::token_major>;
    }
    GGML_ABORT("invalid MoE hot-cache worklist order");
}
...
static ggml_tensor * llama_moe_hot_cache_build_lora_mm_id(
        ...
        ggml_tensor * ids,
        uint32_t flags,
        ggml_tensor * input_row_ids = nullptr) {
    ggml_tensor * res = input_row_ids != nullptr
        ? llama_moe_hot_cache_ggml_mul_mat_id_indirect(graph.ctx0, w, cur, ids, input_row_ids)
        : ggml_mul_mat_id(graph.ctx0, w, cur, ids);
    ...
    for (const auto & lora : *graph.loras) {
        ...
        ggml_tensor * a_cur = input_row_ids != nullptr
            ? llama_moe_hot_cache_ggml_mul_mat_id_indirect(graph.ctx0, lw->a, cur, ids, input_row_ids)
            : ggml_mul_mat_id(graph.ctx0, lw->a, cur, ids);
...
static ggml_tensor * llama_moe_hot_cache_build_moe_ffn_with_ids(
     ...
     llm_ffn_op_type   type_op,
            ...
            ggml_backend_t branch_backend = nullptr,
             ...
             ggml_tensor * input_row_ids = nullptr,
             ggml_tensor * reduce_token_ids = nullptr,
                 int64_t   reduce_n_tokens = 0) {
    ...
    const int64_t n_input_rows = cur->ne[1];
    const int64_t n_slots      = selected_experts->ne[1];
    const bool weight_before_ffn = arch == LLM_ARCH_LLAMA4;
    if (weight_before_ffn) {
        input_row_ids = nullptr;
    }
    GGML_ASSERT(n_slots > 0);
    GGML_ASSERT(input_row_ids != nullptr || n_input_rows == n_slots);
    ...
    cur = ggml_reshape_3d(ctx0, cur, n_embd, 1, n_input_rows);

    if (weight_before_ffn) {
        ggml_tensor * repeated = ggml_repeat_4d(ctx0, cur, n_embd, n_expert_used, n_input_rows, 1);
    ...
    if (gate_up_exps) {
        ggml_tensor * gate_up = llama_moe_hot_cache_build_lora_mm_id(
                graph, gate_up_exps, cur, selected_experts, intermediate_flags, input_row_ids);
        ...
        if (up_exps_s) {
            ...
            s = ggml_repeat_4d(ctx0, s, 1, n_expert, n_slots, 1);
        ...
        up = llama_moe_hot_cache_build_lora_mm_id(
                graph, up_exps, cur, selected_experts, intermediate_flags, input_row_ids);
        cb_moe(up, "ffn_moe_up");
        ...
        if (gate_exps) {
            cur = llama_moe_hot_cache_build_lora_mm_id(
                    graph, gate_exps, cur, selected_experts, intermediate_flags, input_row_ids);
            cb_moe(cur, "ffn_moe_gate");
        } else {
            cur = up;
        }
    ...
    if (reduce_token_ids != nullptr) {
        GGML_ASSERT(!weight_before_ffn);
        GGML_ASSERT(n_expert_used_graph == 1);
        GGML_ASSERT(reduce_n_tokens > 0);
        GGML_ASSERT(down_exps_b == nullptr);
        GGML_ASSERT(down_exps_s == nullptr);
        GGML_ASSERT(graph.loras == nullptr || graph.loras->empty());

        experts = llama_moe_hot_cache_ggml_mul_mat_id_token_reduce(
                ctx0,
                down_exps,
                cur,
                selected_experts,
                ...
                weights,
                reduce_n_tokens);
        llama_moe_hot_cache_set_mul_mat_id_flags(experts, output_flags);
        cb_moe(experts, "ffn_moe_down");
        ggml_build_forward_expand(gf, experts);

        ggml_tensor * reduced_out = ggml_view_2d(ctx0, experts, experts->ne[0], experts->ne[1], experts->nb[1], 0);
        cb_moe(reduced_out, "ffn_moe_out");
        ggml_build_forward_expand(gf, reduced_out);
        return reduced_out;
    }
    ...
    for (uint32_t i = 0; i < n_expert_used_graph; ++i) {
        cur_experts[i] = ggml_view_2d(ctx0, experts, n_embd, n_slots, experts->nb[2], i*experts->nb[1]);
...
static ggml_backend_t llama_moe_hot_cache_pp_cold_backend(
    ...
    switch (llama_moe_hot_cache_pp_policy::cold_backend(profile)) {
        ...
        case llama_moe_hot_cache_pp_cold_backend::primary:
            return llama_moe_hot_cache_primary_merge_backend(graph.sched, model, il);
    }

    return graph.backend_cpu;
}

static bool llama_moe_hot_cache_can_use_indirect_cold_inputs(
        const llm_graph_context & graph,
        ggml_backend_t cold_backend,
        int64_t n_tokens) {
    return cold_backend == graph.backend_cpu &&
        llama_moe_hot_cache_pp_policy::indirect_cold_inputs_enabled(
                llama_moe_hot_cache_graph_phase::prompt_processing,
                n_tokens);
}

static bool llama_moe_hot_cache_can_fuse_cold_down_reduce(
        const llm_graph_context & graph,
        ggml_backend_t cold_backend,
        int64_t n_tokens,
        const ggml_tensor * down_exps_b,
        const ggml_tensor * down_exps_s) {
    return cold_backend == graph.backend_cpu &&
        graph.arch != LLM_ARCH_LLAMA4 &&
        down_exps_b == nullptr &&
        down_exps_s == nullptr &&
        (graph.loras == nullptr || graph.loras->empty()) &&
        llama_moe_hot_cache_pp_policy::fused_cold_down_reduce_enabled(
                llama_moe_hot_cache_graph_phase::prompt_processing,
                n_tokens);
}
...
static int32_t llama_moe_hot_cache_lane_src_slot_field(size_t lane) {
    switch (lane) {
...
static int32_t llama_moe_hot_cache_lane_token_id_field(size_t lane) {
    switch (lane) {
...
static bool llama_moe_hot_cache_layer_has_cold_experts(
    ...
    for (int64_t expert = 0; expert < int64_t(cache.expert_lane_map_host.size()); ++expert) {
        const int32_t lane_index = cache.expert_lane_map_host[size_t(expert)];
        if (lane_index < 0 || size_t(lane_index) >= cache.lanes.size()) {
            return true;
        }

        const auto & lane = cache.lanes[size_t(lane_index)];
        if (expert >= int64_t(lane.hot_id_map_host.size())) {
            return true;
        }

        const int32_t hot_id = lane.hot_id_map_host[size_t(expert)];
...
// Returns the largest possible cold slot count for a PP ubatch in this layer.
// This preserves correctness while shrinking cold-only tensors when the layer
// has fewer cold experts than the model top-k.
static int64_t llama_moe_hot_cache_layer_cold_capacity(
        const llama_moe_hot_cache_layer & cache,
        int64_t n_moe_slots,
        int64_t n_tokens) {
    int64_t cold_experts = 0;
    if (cache.expert_lane_map_host.empty()) {
        cold_experts = std::max<int64_t>(0, int64_t(cache.n_expert) - int64_t(cache.n_hot));
    } else {
        for (int64_t expert = 0; expert < int64_t(cache.expert_lane_map_host.size()); ++expert) {
            const int32_t lane_index = cache.expert_lane_map_host[size_t(expert)];
            if (lane_index < 0 || size_t(lane_index) >= cache.lanes.size()) {
    ...
    return std::min<int64_t>(n_moe_slots, cold_experts) * n_tokens;
}
...
static llama_moe_hot_cache_router_selection llama_moe_hot_cache_build_router_selection(
        ...
        const char * node_suffix = nullptr) {
    ggml_context * ctx0 = graph.ctx0;
    const llama_hparams & hparams = graph.hparams;
    ...
    ggml_tensor * selected_experts = nullptr;
    ggml_tensor * weights = nullptr;
    const auto cb_route = [&](ggml_tensor * t, const char * name) {
        if (node_suffix == nullptr || node_suffix[0] == '\0') {
            graph.cb(t, name, il);
            return;
        }

        const std::string suffixed = format("%s_%s", name, node_suffix);
        graph.cb(t, suffixed.c_str(), il);
    };
    ...
    if (llama_moe_hot_cache_uses_prob_router(adapter)) {
        ...
        cb_route(weights, "ffn_moe_weights");
        ...
        if (gating_func == LLAMA_EXPERT_GATING_FUNC_TYPE_SOFTMAX_WEIGHT) {
            ...
            cb_route(weights, "ffn_moe_weights_softmax");
        ...
        if (hparams.expert_weights_norm) {
            ...
            cb_route(weights_sum, "ffn_moe_weights_sum");
            weights_sum = ggml_clamp(ctx0, weights_sum, 6.103515625e-5, INFINITY);
            cb_route(weights_sum, "ffn_moe_weights_sum_clamped");
            weights = ggml_div(ctx0, weights, weights_sum);
            cb_route(weights, "ffn_moe_weights_norm");
            weights = ggml_reshape_3d(ctx0, weights, 1, n_moe_slots, n_tokens);
        }
        ...
        if (hparams.expert_weights_scale != 0.0f && hparams.expert_weights_scale != 1.0f) {
            ...
            cb_route(weights, "ffn_moe_weights_scaled");
        }
...
static ggml_tensor * llama_moe_hot_cache_build_moe_hot_multi_pp_from_logits(
        const llm_graph_context & graph,
        const llama_model & model,
             ggml_tensor * cur,
             ggml_tensor * logits,
...
static ggml_tensor * llama_moe_hot_cache_build_moe_hot_pp_dense_from_logits(
        const llm_graph_context & graph,
        const llama_model & model,
             ggml_tensor * cur,
             ggml_tensor * logits,
...
static ggml_tensor * llama_moe_hot_cache_build_moe_hot_multi_pp_dense_from_logits(
        const llm_graph_context & graph,
        const llama_model & model,
             ggml_tensor * cur,
             ggml_tensor * logits,
...
static ggml_tensor * llama_moe_hot_cache_build_moe_hot_multi_from_logits(
    ...
    struct hot_lane_branch {
        size_t lane_index;
        const llama_moe_hot_cache_layer_lane * lane;
        ggml_tensor * count;
        ggml_tensor * ids;
        ggml_tensor * weights;
        ggml_tensor * inputs;
    ...
    std::vector<hot_lane_branch> hot_branches;
    hot_branches.reserve(cache.lanes.size());
    ...
    ggml_backend_t cold_backend = nullptr;

    if (llama_moe_hot_cache_layer_has_cold_experts(cache)) {
        cold_backend = cparams.warmup ? nullptr : graph.backend_cpu;
    ...
    for (auto & hot_branch : hot_branches) {
        const auto & lane = *hot_branch.lane;

        ggml_tensor * lane_inputs = ggml_repeat_4d(ctx0, cur, n_embd, capacity, 1, 1);
        if (hot_branch.backend != nullptr) {
            ggml_backend_sched_set_tensor_backend(sched, lane_inputs, hot_branch.backend);
        }
        graph.cb(lane_inputs, format("ffn_moe_hot%zu_inputs", hot_branch.lane_index).c_str(), il);
        hot_branch.inputs = lane_inputs;

        const std::string branch_name = format("hot%zu", hot_branch.lane_index);
        ggml_tensor * lane_out = llama_moe_hot_cache_build_moe_ffn_with_ids(
                ...
                branch_name.c_str(),
                hot_branch.backend);
        graph.cb(lane_out, format("ffn_moe_hot%zu_out", hot_branch.lane_index).c_str(), il);

        hot_branch.slots = merge_compact_slots(
                lane_out,
                hot_branch.backend,
    ...
    if (cold_count != nullptr) {
        cold_inputs = ggml_repeat_4d(ctx0, cur, n_embd, capacity, 1, 1);
    ...
    if (parallel_mode != 0 && !cparams.warmup && annotate_parallel_region && !hot_branches.empty()) {
        ...
        std::vector<ggml_tensor *> hot_starts;
        std::vector<ggml_tensor *> hot_ends;
        std::vector<ggml_tensor *> hot_outputs;
        hot_counts.reserve(hot_branches.size());
        hot_starts.reserve(hot_branches.size());
        hot_ends.reserve(hot_branches.size());
        hot_outputs.reserve(hot_branches.size());
        for (const auto & hot_branch : hot_branches) {
            hot_counts.push_back(hot_branch.count);
            hot_starts.push_back(hot_branch.inputs);
            hot_ends.push_back(hot_branch.slots);
            hot_outputs.push_back(hot_branch.slots);
        }
...
// Dense single-lane PP keeps the normal top-k slot grid for the hot RTX branch
// and optionally compacts only the cold CPU side. This avoids the scatter path
// used by the generic PP implementation while preserving the same router slots.
static ggml_tensor * llama_moe_hot_cache_build_moe_hot_pp_dense_from_logits(
        const llm_graph_context & graph,
        const llama_model & model,
             ggml_tensor * cur,
    ...
    const llama_moe_hot_cache_router_selection router = llama_moe_hot_cache_build_router_selection(
            ...
            adapter,
            "pp_dense");
    ggml_tensor * worklist = ggml_map_custom3(
            ctx0,
            worklist_shape,
            router.selected_experts,
            router.weights,
    ...
    const bool pp_hot_dummy_padding = llama_moe_hot_cache_pp_policy::hot_dummy_padding_enabled(
            n_tokens,
            llama_moe_hot_cache_graph_tweaks::hot_dummy_padding());
    const uint32_t hot_mul_mat_id_flags = pp_hot_dummy_padding
        ? LLAMA_MOE_HOT_CACHE_MUL_MAT_ID_FLAG_NONE
        : LLAMA_MOE_HOT_CACHE_MUL_MAT_ID_FLAG_ALLOW_NEGATIVE_IDS;

    std::vector<ggml_tensor *> branch_outputs;
    branch_outputs.reserve(2);

    ggml_tensor * hot_count = view_worklist_count(LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_HOT_COUNT);
    graph.cb(hot_count, "ffn_moe_hot_count_pp_dense", il);
    ...
    hot_ids = ggml_reshape_2d(ctx0, hot_ids, n_moe_slots, n_tokens);
    graph.cb(hot_ids, "ffn_moe_hot_ids_pp_dense", il);
    ggml_build_forward_expand(gf, hot_ids);

    ggml_tensor * hot_weights = ggml_reshape_3d(
            ctx0,
            view_worklist_field(LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_HOT_WEIGHT),
            1,
            n_moe_slots,
            n_tokens);
    graph.cb(hot_weights, "ffn_moe_hot_weights_pp_dense", il);
    ggml_build_forward_expand(gf, hot_weights);

    ggml_tensor * hot_out = llama_moe_hot_cache_build_moe_ffn_with_ids(
            graph,
            cur,
            hot_ids,
            hot_weights,
            ...
            hot_mul_mat_id_flags,
            "hot_pp_dense");
    graph.cb(hot_out, "ffn_moe_hot_pp_dense_out", il);
    ...
    if (has_cold_experts) {
        ...
        ggml_tensor * cold_ids = ggml_cast(ctx0, view_worklist_field(LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_ID), GGML_TYPE_I32);
            cold_ids = ggml_reshape_2d(ctx0, cold_ids, n_moe_slots, n_tokens);
            graph.cb(cold_ids, "ffn_moe_cold_ids_pp_dense", il);
            ggml_build_forward_expand(gf, cold_ids);

            ggml_tensor * cold_weights = ggml_reshape_3d(
                    ctx0,
                    view_worklist_field(LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_WEIGHT),
                    1,
                    n_moe_slots,
                    n_tokens);
            graph.cb(cold_weights, "ffn_moe_cold_weights_pp_dense", il);
            ggml_build_forward_expand(gf, cold_weights);

            ggml_tensor * cold_count = view_worklist_count(LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_COUNT);
            graph.cb(cold_count, "ffn_moe_cold_count_pp_dense", il);
            ggml_build_forward_expand(gf, cold_count);
            ...
            ggml_tensor * cold_out = llama_moe_hot_cache_build_moe_ffn_with_ids(
                    ...
                    "cold_pp_dense",
                    cold_backend);
            graph.cb(cold_out, "ffn_moe_cold_pp_dense_out", il);
            branch_outputs.push_back(cold_out);
    }
    ...
    graph.cb(out, "ffn_moe_out_pp_dense", il);
    return out;
}

// Dense multi-lane PP keeps the original top-k slot grid [k, n_tokens] for
// every branch. Invalid branch slots use negative or dummy IDs with zero
// weights, so the FFN helper can reduce directly to [n_embd, n_tokens] without
// materializing a per-branch scatter tensor.
static ggml_tensor * llama_moe_hot_cache_build_moe_hot_multi_pp_dense_from_logits(
        const llm_graph_context & graph,
        const llama_model & model,
             ggml_tensor * cur,
    ...
    for (size_t lane_index = 0; lane_index < cache.lanes.size(); ++lane_index) {
        ...
        ggml_tensor * lane_count = view_worklist_count(llama_moe_hot_cache_lane_count_field(lane_index));
        graph.cb(lane_count, format("ffn_moe_hot%zu_count_pp_dense", lane_index).c_str(), il);
        ggml_build_forward_expand(gf, lane_count);
        hot_count_total = hot_count_total == nullptr
            ? lane_count
            : ggml_add(ctx0, hot_count_total, lane_count);
        ...
        ggml_tensor * lane_weights = ggml_reshape_3d(
                ctx0,
                view_worklist_field(llama_moe_hot_cache_lane_weight_field(lane_index)),
                1,
                n_moe_slots,
                n_tokens);
        graph.cb(lane_weights, format("ffn_moe_hot%zu_weights_pp_dense", lane_index).c_str(), il);
        ggml_build_forward_expand(gf, lane_weights);
        ...
        const std::string branch_name = format("hot%zu_pp_dense", lane_index);
        ggml_tensor * lane_out = llama_moe_hot_cache_build_moe_ffn_with_ids(
                graph,
                cur,
                lane_ids,
                ...
                branch_name.c_str(),
                lane_backend);
        graph.cb(lane_out, format("ffn_moe_hot%zu_pp_dense_out", lane_index).c_str(), il);
        branch_outputs.push_back(lane_out);
    }
    if (hot_count_total != nullptr) {
        graph.cb(hot_count_total, "ffn_moe_hot_count_pp_dense", il);
        ggml_build_forward_expand(gf, hot_count_total);
    }
...
// Multi-lane prompt processing keeps its own orchestration because multi-token
// worklists need token gathers and per-token branch reductions instead of the
// decode-only direct merge shortcuts.
static ggml_tensor * llama_moe_hot_cache_build_moe_hot_multi_pp_from_logits(
        const llm_graph_context & graph,
        const llama_model & model,
             ggml_tensor * cur,
    ...
    const llama_moe_hot_cache_pp_execution_plan pp_plan = llama_moe_hot_cache_pp_policy::build(
            ...
            n_tokens,
            capacity,
            llama_moe_hot_cache_layer_has_cold_experts(cache),
            n_moe_slots,
            profile);
    const bool pp_hot_dummy_padding = llama_moe_hot_cache_pp_policy::hot_dummy_padding_enabled(
            n_tokens,
            llama_moe_hot_cache_graph_tweaks::hot_dummy_padding());
    const int64_t hot_lane_capacity = llama_moe_hot_cache_pp_policy::hot_lane_capacity(
            n_tokens,
            capacity,
            (int64_t) cache.lanes.size(),
            pp_hot_dummy_padding);
    const bool cap_hot_lanes = hot_lane_capacity < capacity;
    const llama_moe_hot_cache_worklist_order worklist_order =
        cap_hot_lanes ? llama_moe_hot_cache_worklist_order::token_major : pp_plan.worklist_order;
    ...
    const auto scatter_compact_branch_to_token_rows = [&](
            ggml_tensor * branch_out,
            ggml_tensor * branch_src_slots,
            ggml_backend_t branch_backend,
            const char * slots_name,
            const char * reduced_name) {
        ggml_tensor * slots = ggml_repeat_4d(ctx0, cur, n_embd, capacity, 1, 1);
        if (branch_backend != nullptr) {
            ggml_backend_sched_set_tensor_backend(sched, slots, branch_backend);
        }
        slots = ggml_scale(ctx0, slots, 0.0f);
        if (branch_backend != nullptr) {
            ggml_backend_sched_set_tensor_backend(sched, slots, branch_backend);
        }
        slots = ggml_pad(ctx0, slots, 0, 1, 0, 0);
        if (branch_backend != nullptr) {
            ggml_backend_sched_set_tensor_backend(sched, slots, branch_backend);
        }
        slots = ggml_set_rows(ctx0, slots, branch_out, branch_src_slots);
        if (branch_backend != nullptr) {
            ggml_backend_sched_set_tensor_backend(sched, slots, branch_backend);
        }
        slots = ggml_view_2d(ctx0, slots, n_embd, capacity, slots->nb[1], 0);
        if (branch_backend != nullptr) {
            ggml_backend_sched_set_tensor_backend(sched, slots, branch_backend);
        }
        slots = ggml_reshape_3d(ctx0, slots, n_embd, n_moe_slots, n_tokens);
        if (branch_backend != nullptr) {
            ggml_backend_sched_set_tensor_backend(sched, slots, branch_backend);
        }
        graph.cb(slots, slots_name, il);
        ggml_build_forward_expand(gf, slots);
        return reduce_slots_in_branch(slots, branch_backend, reduced_name);
    };
    ...
    for (size_t lane_index = 0; lane_index < cache.lanes.size(); ++lane_index) {
        ...
        ggml_tensor * lane_src_slots = ggml_cast(ctx0, view_worklist_field(llama_moe_hot_cache_lane_src_slot_field(lane_index), hot_lane_capacity), GGML_TYPE_I32);
        graph.cb(lane_src_slots, format("ffn_moe_hot%zu_src_slots", lane_index).c_str(), il);
        ggml_build_forward_expand(gf, lane_src_slots);

        ggml_tensor * lane_token_ids = ggml_cast(ctx0, view_worklist_field(llama_moe_hot_cache_lane_token_id_field(lane_index), hot_lane_capacity), GGML_TYPE_I32);
        graph.cb(lane_token_ids, format("ffn_moe_hot%zu_token_ids", lane_index).c_str(), il);
        ggml_build_forward_expand(gf, lane_token_ids);

        ggml_tensor * lane_weights = ggml_reshape_3d(ctx0, view_worklist_field(llama_moe_hot_cache_lane_weight_field(lane_index), hot_lane_capacity), 1, 1, hot_lane_capacity);
        graph.cb(lane_weights, format("ffn_moe_hot%zu_weights_compact", lane_index).c_str(), il);
        ggml_build_forward_expand(gf, lane_weights);
        ...
        ggml_tensor * lane_inputs = ggml_get_rows(ctx0, cur, lane_token_ids);
        ...
        branch_outputs.push_back(scatter_compact_branch_to_token_rows(
                lane_out,
                lane_src_slots,
                lane_backend,
                format("ffn_moe_hot%zu_slots", lane_index).c_str(),
                format("ffn_moe_hot%zu_slots_reduced", lane_index).c_str()));
    }
    if (hot_count_total != nullptr) {
        graph.cb(hot_count_total, "ffn_moe_hot_count", il);
        ggml_build_forward_expand(gf, hot_count_total);
    ...
    if (llama_moe_hot_cache_layer_has_cold_experts(cache)) {
        ...
        const bool use_indirect_cold_inputs =
            llama_moe_hot_cache_can_use_indirect_cold_inputs(graph, cold_backend, n_tokens);
        const bool use_fused_cold_down_reduce =
            llama_moe_hot_cache_can_fuse_cold_down_reduce(
                    graph,
                    cold_backend,
                    n_tokens,
                    layer.ffn_down_exps_b,
        ...
        ggml_tensor * cold_inputs = cur;
        ggml_tensor * cold_input_row_ids = nullptr;
        if (use_indirect_cold_inputs) {
            cold_input_row_ids = cold_token_ids;
        } else {
        ...
        ggml_tensor * cold_out = llama_moe_hot_cache_build_moe_ffn_with_ids(
                ...
                true,
                cold_input_row_ids,
                use_fused_cold_down_reduce ? cold_token_ids : nullptr,
                use_fused_cold_down_reduce ? n_tokens : 0);
        graph.cb(cold_out, "ffn_moe_cold_out", il);

        branch_outputs.push_back(use_fused_cold_down_reduce
            ? cold_out
            : scatter_compact_branch_to_token_rows(
                    cold_out,
                    cold_src_slots,
                    cold_backend,
                    "ffn_moe_cold_slots",
                    "ffn_moe_cold_slots_reduced"));
    }
...
static ggml_tensor * llama_moe_hot_cache_build_moe_hot_from_logits(
    ...
    const bool perf_or_parallel_counts =
        !is_warmup_phase && (perf_expert_counts || (parallel_mode != 0 && annotate_parallel_region));
    ...
    if (cache.n_hot != cache.n_expert) {
        ...
        const bool use_indirect_cold_inputs =
            cold_token_ids != nullptr &&
            !cold_shared_input_row &&
            llama_moe_hot_cache_can_use_indirect_cold_inputs(graph, cold_branch_backend, n_tokens);
        const bool use_fused_cold_down_reduce =
            branch_reduce_merge &&
            cold_token_ids != nullptr &&
            cold_weights != nullptr &&
            llama_moe_hot_cache_can_fuse_cold_down_reduce(
                    graph,
                    cold_branch_backend,
                    n_tokens,
        ...
        if (!use_indirect_cold_inputs && cold_branch_backend != nullptr) {
        ...
        if (use_fused_cold_down_reduce) {
            cold_parallel_output = cold_out;
        } else if (decode_direct_merge) {
            ...
            ggml_tensor * cold_slots_zero_src = nullptr;
            if (cold_shared_input_row) {
                cold_slots_zero_src = cold_out;
            } else if (use_indirect_cold_inputs) {
                cold_slots_zero_src = ggml_repeat_4d(ctx0, cur, n_embd, capacity, 1, 1);
            } else {
                cold_slots_zero_src = cold_inputs;
            }
        ...
        if (branch_reduce_merge && !use_fused_cold_down_reduce) {
...
ggml_tensor * llama_model_qwen35moe::graph::build_layer_ffn_hot(ggml_tensor * cur, const int il) {
    ...
    if (cache.n_hot != cache.n_expert) {
        ...
        const bool use_indirect_cold_inputs =
            cold_token_ids != nullptr &&
            !cold_shared_input_row &&
            llama_moe_hot_cache_can_use_indirect_cold_inputs(*this, cold_branch_backend, n_tokens);
        const bool use_fused_cold_down_reduce =
            branch_reduce_merge &&
            cold_token_ids != nullptr &&
            cold_weights != nullptr &&
        ...
        if (use_fused_cold_down_reduce) {
            ...
            ggml_tensor * cold_slots_zero_src = use_indirect_cold_inputs
                ? ggml_repeat_4d(ctx0, cur, n_embd, capacity, 1, 1)
                : cold_inputs;
            cold_slots = ggml_scale(ctx0, cold_slots_zero_src, 0.0f);
            if (cold_branch_backend != nullptr) {
