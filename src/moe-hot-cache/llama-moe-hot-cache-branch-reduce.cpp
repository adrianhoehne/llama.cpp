...
static void llama_moe_hot_cache_compact_cold_weighted_reduce_op(
        ggml_tensor * dst,
        const ggml_tensor * shape,
        const ggml_tensor * branch_out,
        const ggml_tensor * worklist,
    ...
    llama_moe_hot_cache_reduce_cold_weighted_token_rows(dst, branch_out, worklist, ith, nth);
}
...
void llama_moe_hot_cache_reduce_cold_weighted_token_rows(
        ggml_tensor * dst,
        const ggml_tensor * branch_out,
        const ggml_tensor * worklist,
        int ith,
    ...
    for (int64_t slot = 0; slot < n_prefix; ++slot) {
        ...
        const float weight = worklist_field_value(worklist, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_WEIGHT, slot);
        float * dst_row = (float *) ((char *) dst->data + i0*dst->nb[0] + token*dst->nb[1]);
        const float * src_row = (const float *) ((const char *) branch_out->data + i0*branch_out->nb[0] + slot*branch_out->nb[1]);
        for (int64_t i = 0; i < n_local_embd; ++i) {
            dst_row[i] += src_row[i]*weight;
        }
    }
}
...
ggml_tensor * llama_moe_hot_cache_build_compact_cold_weighted_reduce(
        const llm_graph_context & graph,
        ggml_tensor * branch_out,
        ggml_tensor * worklist,
        ggml_backend_t backend,
    ...
    ggml_tensor * reduced = ggml_map_custom3(
            ...
            branch_out,
            worklist,
            llama_moe_hot_cache_compact_cold_weighted_reduce_op,
            std::max(1, n_tasks),
            nullptr);
    if (backend != nullptr) {
        ggml_backend_sched_set_tensor_backend(graph.sched, reduced, backend);
