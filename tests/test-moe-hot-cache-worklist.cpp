...
static void test_build_worklist_multi_lane_routes_by_lane_map() {
    auto ctx = make_ctx();
    require(ctx != nullptr);
    ...
    set_selected(selected, 0, 0, 0);
    set_selected(selected, 1, 0, 1);
    set_selected(selected, 0, 1, 2);
    set_selected(selected, 1, 1, 3);

    set_weight(weights, 0, 0, 0.10f);
    set_weight(weights, 1, 0, 0.20f);
    set_weight(weights, 0, 1, 0.30f);
    set_weight(weights, 1, 1, 0.40f);

    llama_moe_hot_cache_layer layer;
    layer.n_expert = 4;
    layer.expert_weights_scale = 1.0f;
    layer.expert_lane_map_host = { 0, 1, -1, 1 };
    layer.lanes.resize(2);
    layer.lanes[0].n_expert = 4;
    layer.lanes[0].n_hot = 1;
    layer.lanes[0].hot_id_map_host = { 0, -1, -1, -1 };
    layer.lanes[1].n_expert = 4;
    layer.lanes[1].n_hot = 2;
    layer.lanes[1].hot_id_map_host = { -1, 0, -1, 1 };

    llama_moe_hot_cache_build_worklist(packed, selected, weights, layer, 0, 1);

    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_HOT_ID, 0) == 0.0f);
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_HOT_SRC_SLOT, 0) == 0.0f);
    require_close(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_HOT_WEIGHT, 0), 0.10f);
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_HOT_EXPERT_ID, 0) == 0.0f);

    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_HOT1_ID, 0) == 0.0f);
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_HOT1_SRC_SLOT, 0) == 1.0f);
    require_close(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_HOT1_WEIGHT, 0), 0.20f);
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_HOT1_EXPERT_ID, 0) == 1.0f);

    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_HOT1_ID, 1) == 1.0f);
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_HOT1_SRC_SLOT, 1) == 3.0f);
    require_close(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_HOT1_WEIGHT, 1), 0.40f);
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_HOT1_EXPERT_ID, 1) == 3.0f);

    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_ID, 0) == 2.0f);
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_SRC_SLOT, 0) == 2.0f);
    require_close(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_WEIGHT, 0), 0.30f);

    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_HOT_COUNT, 0) == 1.0f);
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_HOT1_COUNT, 0) == 2.0f);
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_HOT2_COUNT, 0) == 0.0f);
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_COUNT, 0) == 1.0f);
}

static void test_build_worklist_token_aligned_single_lane() {
    auto ctx = make_ctx();
    require(ctx != nullptr);
    ...
    llama_moe_hot_cache_build_worklist(
            ...
            0,
            1,
            llama_moe_hot_cache_worklist_order::token_aligned);
    ...
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_HOT_ID, 2) == expected_hot_padding);
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_ID, 2) == 2.0f);
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_SRC_SLOT, 2) == 2.0f);
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_TOKEN_ID, 2) == 1.0f);
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_WEIGHT, 2) == 0.21f);

    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_HOT_ID, 3) == 0.0f);
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_HOT_SRC_SLOT, 3) == 3.0f);
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_HOT_TOKEN_ID, 3) == 1.0f);
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_ID, 3) == -1.0f);

    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_HOT_COUNT, 0) == 3.0f);
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_COUNT, 0) == 3.0f);
}
...
static void test_build_worklist_multi_lane_cold_compact_capacity() {
    auto ctx = make_ctx();
    require(ctx != nullptr);

    const int32_t n_expert_used = 4;
    const int32_t n_tokens = 2;
    const int32_t cold_capacity = 2*n_tokens;

    ggml_tensor * selected = ggml_new_tensor_2d(ctx.get(), GGML_TYPE_I32, n_expert_used, n_tokens);
    ggml_tensor * weights = ggml_new_tensor_3d(ctx.get(), GGML_TYPE_F32, 1, n_expert_used, n_tokens);
    ggml_tensor * packed = ggml_new_tensor_2d(ctx.get(), GGML_TYPE_F32, cold_capacity, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COUNT);

    set_selected(selected, 0, 0, 0);
    set_selected(selected, 1, 0, 2);
    set_selected(selected, 2, 0, 4);
    set_selected(selected, 3, 0, 3);
    set_selected(selected, 0, 1, 5);
    set_selected(selected, 1, 1, 2);
    set_selected(selected, 2, 1, 1);
    set_selected(selected, 3, 1, 3);

    for (int32_t token = 0; token < n_tokens; ++token) {
        for (int32_t iex = 0; iex < n_expert_used; ++iex) {
            set_weight(weights, iex, token, 0.10f*float(token + 1) + 0.01f*float(iex + 1));
        }
    }

    llama_moe_hot_cache_layer layer;
    layer.n_expert = 6;
    layer.expert_lane_map_host = { 0, 0, -1, -1, 1, 1 };
    layer.lanes.resize(2);
    layer.lanes[0].n_expert = 6;
    layer.lanes[0].n_hot = 2;
    layer.lanes[0].hot_id_map_host = { 0, 1, -1, -1, -1, -1 };
    layer.lanes[1].n_expert = 6;
    layer.lanes[1].n_hot = 2;
    layer.lanes[1].hot_id_map_host = { -1, -1, -1, -1, 0, 1 };
    ...
    llama_moe_hot_cache_build_worklist(
            ...
            0,
            1,
            llama_moe_hot_cache_worklist_order::cold_compact);

    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_COUNT, 0) == 4.0f);

    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_ID, 0) == 2.0f);
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_SRC_SLOT, 0) == 1.0f);
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_TOKEN_ID, 0) == 0.0f);
    require_close(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_WEIGHT, 0), 0.12f);

    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_ID, 1) == 2.0f);
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_SRC_SLOT, 1) == 5.0f);
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_TOKEN_ID, 1) == 1.0f);
    require_close(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_WEIGHT, 1), 0.22f);

    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_ID, 2) == 3.0f);
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_SRC_SLOT, 2) == 3.0f);
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_TOKEN_ID, 2) == 0.0f);
    require_close(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_WEIGHT, 2), 0.14f);

    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_ID, 3) == 3.0f);
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_SRC_SLOT, 3) == 7.0f);
    require(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_TOKEN_ID, 3) == 1.0f);
    require_close(get_worklist_field(packed, LLAMA_MOE_HOT_CACHE_WORKLIST_FIELD_COLD_WEIGHT, 3), 0.24f);
}

static void test_build_worklist_single_lane_cold_compact_capacity() {
    auto ctx = make_ctx();
    require(ctx != nullptr);
    ...
    layer.n_expert = 6;
    layer.n_hot = 4;
    layer.hot_id_map_host = { 0, 1, -1, -1, 2, 3 };
...
int main() {
    ...
    test_build_worklist_token_aligned_single_lane();
    ...
    test_build_worklist_multi_lane_cold_compact_capacity();
    test_build_worklist_single_lane_cold_compact_capacity();
    return 0;
}
